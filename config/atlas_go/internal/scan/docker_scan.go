			...
			if ns, ok := portData[0]["NetworkSettings"].(map[string]interface{}); ok {
				if pmap, ok := ns["Ports"].(map[string]interface{}); ok {
					for port, val := range pmap {
						portFields := strings.Split(port, "/")
						proto := "tcp"
						portNum := 0
						if len(portFields) > 0 {
							portNum, _ = strconv.Atoi(portFields[0])
						}
						if len(portFields) > 1 {
							proto = portFields[1]
						}
						if val == nil {
							...
							remotePorts = append(remotePorts, RemotePort{Port: portNum, Protocol: proto, State: "internal"})
						} else {
							...
							if arrOK && len(arr) > 0 {
								...
								if entryOK {
									...
									if hIpOK && hPortOK {
										...
										remotePorts = append(remotePorts, RemotePort{Port: portNum, Protocol: proto, State: "host"})
									} else {
										ports = append(ports, fmt.Sprintf("%s (internal)", port))
										remotePorts = append(remotePorts, RemotePort{Port: portNum, Protocol: proto, State: "internal"})
									}
...
func DockerScan(opts DockerScanOptions) error {
	...
	if err := updateDockerDB(allContainers); err != nil {
		return err
	}
	return emitHosts(containersToHostRecords(allContainers), opts.Remote)
}

func containersToHostRecords(containers []DockerContainer) []HostRecord {
	hosts := make([]HostRecord, 0, len(containers))
	now := time.Now()
	for _, c := range containers {
		if c.IP == "" {
			continue
		...
		host := HostRecord{
			...
			PortSummary:   c.Ports,
			Ports:         c.RemotePorts,
			NetworkName:   c.NetName,
			InterfaceName: c.NetName,
			NextHop:       c.NextHop,
			LastSeen:      now,
			OnlineStatus:  status,
			Metadata:      meta,
